    ...
    def compute_densityfeature(self, xyz_sampled, mask):
        ...
        for idx_plane in range(len(self.density_plane)):
            ...
            if mask == None:                                            
              ...
              sigma_mask = mask[idx_plane][..., None]
              sigma_feature = sigma_feature + torch.sum((plane_coef_point*sigma_mask) * (line_coef_point*sigma_mask), dim=0)
            ...
            if mask == None:
                ...
                plane_coef_point.append(
                    F.grid_sample(
                        ...
                        align_corners=True
                    ).view(-1, *xyz_sampled.shape[:1]) * mask[idx_plane][..., None]
                )
...
class TensorCP(TensorBase):
    ...
    def compute_densityfeature(self, xyz_sampled, mask):
        ...
        if mask == None:
          ...
          sigma_feature = torch.sum(line_coef_point*mask[0][..., None], dim=0)
    ...
    def compute_appfeature(self, xyz_sampled, mask):
        ...
        if mask == None:
            ...
            return self.basis_mat((line_coef_point*mask[0][..., None]).T)
